...
namespace Radzen.Blazor
{
    ...
    /// </code>
    /// </example>
    public partial class RadzenDropDownAutoComplete<TValue> : DropDownBase<TValue>
    {
        /// <summary>
		...
		[Parameter]
        public Action<DropDownAutoCompleteItemRenderEventArgs<TValue>> ItemRender { get; set; }

        internal DropDownAutoCompleteItemRenderEventArgs<TValue> ItemAttributes(RadzenDropDownAutoCompleteItem<TValue> item)
        {
            var disabled = !string.IsNullOrEmpty(DisabledProperty) ? GetItemOrValueFromProperty(item.Item, DisabledProperty) : false;

            var args = new DropDownAutoCompleteItemRenderEventArgs<TValue>() 
            { 
                DropDown = this, 
                Item = item.Item, 
        ...
        {
            builder.OpenComponent(0, typeof(RadzenDropDownAutoCompleteItem<TValue>));
